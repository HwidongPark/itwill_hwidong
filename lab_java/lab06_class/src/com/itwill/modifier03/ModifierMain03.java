...
/*
 *  데이터 캡슐화(encapsulation) - 약 먹을 때 약 성분 다 알필요 없다. 그냥 알맞는 약 먹으면 된다. 약 성분에 해당하는 필드를 공개하지 않고 사용할 수 있도록 하는게 캡슐화이다.
 *  필드(멤버 변수)들을 private으로 선언해서 클래스 외부에서는 보이지 않도록 감추고,
 *  대신에 필요한 경우에 한에서 public으로 공개한 메서드를 제공해서
 *  간접적으로 필드들의 값을 읽거나 수정하는 것을 허용하는 객체지향 프로그래밍 방법
 *  데이터의 보안과 무결성을 유지하기 위해서 캡슐화 사용
