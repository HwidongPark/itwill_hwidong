...
public class MemberDaoImpl implements MemeberDao {
    ...
    public static MemberDaoImpl getInstance() {
        if (instance == null) { // MemberDaoImpl 타입 객체가 생성돼 있지 않은 경우
    ...
    private int count = 0; // 현재까지 배열에 저장된 원소 개수. 배열에 원소가 저장될 때 마다 ++.
    ...
    /**
     * 아규먼트 index가 유효한 범위의 인덱스인지를 검사.
     * index >= 0 이고. index가 배열에 저장된 원소 개수(count)보다 작으면 true,
     * 그렇지 않으면 false.
     * 
     * @param index 유효한지 검사할 인덱스. 정수(int) 타입.
