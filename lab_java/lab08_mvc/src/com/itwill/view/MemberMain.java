...
public class MemberMain {
    ...
    public int selectMainMenu() {
        System.out.println("\n-----------------------------------------------------------------------------");
        System.out.println("[0]종료 | [1] 전체 목록 | [2]새 회원 등록 | [3]인덱스 검색 | [4]비밀번호 변경");
        System.out.println("\n-----------------------------------------------------------------------------");
        System.out.print("선택 >>> ");
...
//        }
    ...
    public void updatePassword() {
        ...
        if (!dao.isValidIndex(index)) {
            System.out.println("해당 인덱스에는 회원 정보가 없습니다.");
            return; // 메서드 종료.. void 메서드는 이렇게 종료하면 됨
        } 
        System.out.print("새 비밀번호 >>> ");
        String newPassword = scanner.nextLine();
        
        int result = dao.update(index, newPassword);
        if (result == 1) {
