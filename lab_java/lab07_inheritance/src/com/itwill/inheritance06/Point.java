...
public class Point {
    ...
    // Object에서 상속받은 toString을 재정의(override)하게되면 콘솔 출력 내용을 변경할 수 있음.
    @Override
    ...
    // Object 클래스에서는 equals 메서드는 실제 힙에서 생성된 객체가 같으면 true, 그렇지 않으면 false를 리턴.
    // Object 클래스에서 상속받은 equals 메서드를 재정의(override)하면서
    // 두 객체의 같음을 새로 정의할 수 있음.
    // 예를 들면 2차원 평면의 좌표 값이 같으면 true, 그렇지 않으면 false.
    @Override
    ...
    // equals를 override할 경우 무조건 hashCode도 override해야 한다.  ***************
    ...
    // equals가 true이면 hashCode가 같도록 정수(int)를 리턴하면 됨
    @Override
    public int hashCode() {
        return (int) x + (int) y;
    }
