...
/*
 * InputStream, OutputStream과 같은 클래스 객체들은 사용한 이후에 반드시 close() 메서드를 호출해서
 * 사용했던 리소스들을 해제해야 함.
 * try-catch-finally 구문에서는
 * 리소스 생성은 try 블록에서 하고, 리소스 해제는 finally 블록에서 작성.
 * 리소스를 생성하면 자동으로 close() 메서드를 호출해 주는 try-with-resource 문법이 생겨남
 * 
 * try (리소스 생성;) {
 *     정상적인 상황에서 실행할 코드 블록;
 * } catch(예외 타입 변수 선언) {
 ...
 * }
 * 
 * try(리소스 생성) -> 여기서는 close() 메서드를 가지고 있는 리소스들만 생성 가능.
 *     
 */
