 ...
 * 
 * 현재 작업 디렉토리(CWD, current working directory): 현재 프로그램(프로세스)가 실행되는 디렉토리 위치.
 ...
 * 상대 경로(relative path)
 * - 현재 작업 디렉토리(CWD)를 기준으로 파일 또는 폴더 위치를 찾아가는 방식.
 * - (Windows 예) data\ansi.txt
 * - (MaxOS, Linux 예) data/ansi.txt
...
public class FileMain09 {
    ...
    public static void main(String[] args) {
        ...
        // 현재는 OS 구분 없이 "data/ansi.txt"로 표현해도 됨
        ...
        // File 클래스:
        // 파일(txt, jpg, mp4, pptx, ...)과 폴더(디렉토리)에 관련된 기능들을 가지고 있는 클래스. 즉, 넓은 의미로 파일, 폴더 모두 포함
        // File 객체 생성 - 파일(파일&폴더)을 관리하는 객체 생성.
        File f = new File(path3);
        System.out.println(f);   // File 클래스는 toString 메서드를 재정의.
        System.out.println("경로: " + f.getPath());
        // -> File 객체를 생성할 때 상대경로로 생성된 경우에는, getPath()는 상대경로를 return. 
        // -> File 객체를 생성할 때 절대경로로 생성된 경우에는, getPath()는 절대경로를 return.
        System.out.println("절대 경로: " + f.getAbsolutePath());
        System.out.println("생성여부: " + f.exists());
        System.out.println("파일 여부: " + f.isFile());
        System.out.println("폴더 여부: " + f.isDirectory());
        System.out.println("파일 크기: " + f.length() + "Bytes");
        
        // 새 폴더 만들기
        File newFolder = new File("data", "test");  // new File(상위 폴더, 하위 폴더/파일)
        if (newFolder.exists()) {
