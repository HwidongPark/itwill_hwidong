...
// 특정 클래스를 상속(extends)하거나, 특정 인터페이스를 구현(implements)할 필요 없이
// (상위 타입의 특정 메서드를 반드시 재정의 할 필요가 없는) 평범한 자바 객체.
// 스프링 프레임워크는 POJO로 작성된 클래스를 컨트롤러로 사용할 수 있음.
// (비교) Servlet을 상속받는 클래스에서는 doGet(request, response) 또는 doPost(request, response)를 반드시 Override.

@Slf4j  // private static final Logger log 변수를 선언하고 초기화를 해줌.
@Controller // 디스페쳐 서블릿에세 컨트롤러 컴포넌트임을 알려줌.
// -> 디스패쳐 서블릿이 객체를 생성, 관리 - 필요할 때 메서드를 호출
public class ExampleController {
    
    @GetMapping("/")    // context root로 Get방식의 요청이 왔을 때 호출되는 메서드
    ...
    @GetMapping("/ex2")
    public void example2(/*@RequestParam(name="username")*/String username, /*@RequestParam(name="age", defaultValue = "0")*/int age, Model model) {
        // @RequestParam: 디스패쳐 서블릿에서 요청 파라미터를 분석해서(request.getParameter(name))해서ㅐ]
        // 컨트롤러 메서드를 호출할 때 아규먼트를 전달할 수 있음.
        // name 속성: 요청 파라미터 이름
        // defaultValue 속성: 요청 파라미터 값이 없을 때(빈 문자열일 때) 사용할 기본값. 
        log.debug("example2()");
        
        ExampleDto dto = ExampleDto.builder().username(username).age(age).build();
        model.addAttribute(dto);
        
        log.debug("example2(name={}, age={})", username, age);
           
    }
    
    
    @PostMapping("/ex3")
    public String example3(@ModelAttribute(name = "exampleDto") ExampleDto dto, Model model) {
...
//        model.addAttribute("dto", dto);
    ...
    public ExampleDto rest2() {
        ...
        // JSON(JavaScriptObjectNotation) 문자열로 변환 후 클라이언트로 응답.
