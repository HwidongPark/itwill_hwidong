 ...
 */
 ...
 document.addEventListener('DOMContentLoaded', function () {
    ...
    console.log(person.name);   // 참조 연산자 사용
    console.log(person['age']);    // 인덱스 연산자 사용
    ...
    // for-in 구문에서 객체를 사용할 수 없음. -> 객체의 property 이름들을 순회(iteration)
    for(let x in person) {
        console.log(x, ':', person[x]);
    }
    
    // 자바스크립트의 객체는 객체가 만들어진 이후에 프로퍼티들을 추가할 수 있음.
    ...
    // 바깥에 정의된 함수를 가져다가 쓸 수 있음
    const obj = {
        testMethod: function() {
            console.log('메서트 testMethod ----');
            test();
        }
    };
