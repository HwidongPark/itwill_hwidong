...
public class CommentRestController {
    ...
    public ResponseEntity<Integer> registerComment(@RequestBody CommentRegisterDto dto) {
        ...
        //-> jackson-databind 라이브러리: JSON 문자열을 자바 객체로 변환. (직렬화)
        //-> jackson-databind 동작 방식: 클래스의 기본 생성자 호출 -> setter 매서드 호출 (역직렬화)
        ...
        return ResponseEntity.ok(result);   //-> 응답코드 (200:success)과 데이터 result를 클라이언트로 전송.
    }
    
    @GetMapping("/all/{postId}")
    public ResponseEntity<List<CommentListItemDto>> getAllComments(@PathVariable long postId) {
        // @PathVariable: 요청 주소의 일부가 변수처럼 변할 때,
    ...
    public ResponseEntity<CommentListItemDto> getCommentById(@PathVariable long id) {
        ...
        // 서비스 계층의 메서드를 호출해서 응답을 보낼 DTO객체를 읽어옴.
    ...
    @PutMapping("/{id}")
    public ResponseEntity<Integer> updateCommentById(@RequestBody CommentUpdateDto dto) {
        log.debug("updateCommentById(id={}, ctext={})", dto.getId(), dto.getCtext());
        
        log.debug("CommentUpdateDto={}", dto);
        

        // 서비스 계층 메서드 호출해서 응답 보낼 DTO객체를 읽어옴
